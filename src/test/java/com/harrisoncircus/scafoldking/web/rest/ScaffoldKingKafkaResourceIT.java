...
import com.harrisoncircus.scafoldking.config.KafkaProperties;
...
class ScaffoldKingKafkaResourceIT {
    ...
    void setup() {
        ...
        ScaffoldKingKafkaResource kafkaResource = new ScaffoldKingKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/scaffold-king-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/scaffold-king-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
